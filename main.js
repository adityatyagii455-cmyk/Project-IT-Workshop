...
(function() {
    ...
    function smoothScrollToSection(e) {
        ...
        if (!targetId || !targetId.startsWith('#')) {
            return; // allow normal navigation (e.g., admin.html)
        }
        e.preventDefault();
    ...
    // Event listeners for gallery items (initial)
    galleryItems.forEach(item => enhanceGalleryItem(item));

    function enhanceGalleryItem(item) {
        if (item.getAttribute('data-enhanced') === 'true') return;
    ...
    // Expose a hook to enhance dynamically added items
    window.initializeDynamicGalleryItems = function() {
        document.querySelectorAll('.gallery-item').forEach(enhanceGalleryItem);
    };
